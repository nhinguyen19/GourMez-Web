...
function loginFromSocialCallBack($socialUser) {
    $con=connectdb();
    $result = mysqli_query($con, "Select `id`,`user_name`,`email`,`fullname` from `user` WHERE `email` ='" . $socialUser['email'] . "'");
    
    if ($result->num_rows == 0) {
        $result = mysqli_query($con, "INSERT INTO `user` (`fullname`,`email`, `verify_status`, `created_at`, `last_updated`) VALUES ('" . $socialUser['name'] . "', '" . $socialUser['email'] . "', 1, " . time() . ", '" . time() . "');");
        if (!$result) {
